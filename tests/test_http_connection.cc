...
// void test_pool() {
//     yuyu::http::HttpConnectionPool::ptr pool(new yuyu::http::HttpConnectionPool(
//                 "www.sylar.top", "", 80, false, 10, 1000 * 30, 5));
// 
//     yuyu::IOManager::GetThis()->addTimer(1000, [pool](){
//             auto r = pool->doGet("/", 300);
//             YUYU_LOG_INFO(g_logger) << r->toString();
//     }, true);
// }

void run() {
    yuyu::Address::ptr addr = yuyu::Address::LookupAnyIPAddress("www.sylar.top:80");
    if(!addr) {
        YUYU_LOG_INFO(g_logger) << "get addr error";
        return;
    }
    ...
    if(!rt) {
        YUYU_LOG_INFO(g_logger) << "connect " << *addr << " failed";
        return;
    }

    yuyu::http::HttpConnection::ptr conn(new yuyu::http::HttpConnection(sock));
    yuyu::http::HttpRequest::ptr req(new yuyu::http::HttpRequest);
    req->setPath("/blog/");
    req->setHeader("host", "www.sylar.top");
    YUYU_LOG_INFO(g_logger) << "req:" << std::endl
        << *req;
    ...
    if(!rsp) {
        YUYU_LOG_INFO(g_logger) << "recv response error";
        return;
    }
    YUYU_LOG_INFO(g_logger) << "rsp:" << std::endl
        << *rsp;
    ...
    auto r = yuyu::http::HttpConnection::DoGet("http://www.sylar.top/blog/", 300);
    YUYU_LOG_INFO(g_logger) << "result=" << r->result
        << " error=" << r->error
...
void test_https() {
    auto r = yuyu::http::HttpConnection::DoGet("http://www.baidu.com/", 300, {
...
void test_data() {
    yuyu::Address::ptr addr = yuyu::Address::LookupAny("www.baidu.com:80");
    auto sock = yuyu::Socket::CreateTCP(addr);
...
//    yuyu::http::HttpResponseParser parser;
...
//
//    yuyu::ZlibStream::ptr stream = yuyu::ZlibStream::CreateGzip(false);
